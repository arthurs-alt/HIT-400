...
def login(request):
    if (User.objects.filter(email=request.POST['login_email']).exists()):
        ...
        #bcrypt.hashpw(request.POST['login_password'].encode('utf8'), bcrypt.gensalt())

        request.session['id'] = user.id
        request.session['name'] = user.first_name
        request.session['surname'] = user.last_name
        messages.add_message(request,messages.INFO,'Welcome to ZRP Quick Response system '+ user.first_name+' '+user.last_name)
